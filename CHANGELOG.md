...
- 后台管理页由顶部 Tab 调整为左侧栏 Tab 导航，右侧按分类展示内容。
- 导航分类整理为概览、数据库治理、采集内容、Cron / 队列、前台与缓存、趋势与日志、设置。
- 左侧导航新增分类模块数量显示，方便判断每个分类下的信息量。
- 小屏幕下左侧栏自动转为网格导航，减少后台移动端/窄屏使用阻塞。

### Safety

- 本版本只调整后台界面呈现，不改变诊断、清理、缓存、队列或数据库维护逻辑。

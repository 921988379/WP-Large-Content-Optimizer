...
3.8.1 主要整理插件后台诊断页的信息架构：

- 将原顶部模块按钮调整为左侧栏 Tab 导航，右侧按分类展示内容。
- 导航归类为概览、数据库治理、采集内容、Cron / 队列、前台与缓存、趋势与日志、设置。
- 左侧导航显示每个分类下的模块数量，减少长页面查找成本。
- 移动端自动降级为网格导航，避免小屏幕遮挡内容。

本版本只调整后台管理界面呈现，不改变清理、诊断、缓存或队列处理逻辑。
